...
impl Store {
    ...
    /// Routines whose instruction matches, in the order they will next fire.
    fn matching_routines(&self, pattern: &str, limit: u32) -> Result<Vec<Routine>, StoreError> {
        let conn = self.conn()?;
        let mut stmt = conn.prepare(
            r"SELECT id,agent_id,what,every_secs,next_run_at,last_run_at,created_at
                FROM routines WHERE what LIKE ?1 ESCAPE '\'
...
mod tests {
    ...
    fn searchable() -> Searchable {
        ...
        f.store.create_routine(writer.id, "post the budget summary", Some(3600), 5_000).unwrap();
        f.store.create_routine(writer.id, "water the plants", None, 4_000).unwrap();
    ...
    #[test]
    fn a_routine_is_found_by_what_it_says() {
        let s = searchable();
        let hits = s.f.store.search("budget", 20).unwrap();
